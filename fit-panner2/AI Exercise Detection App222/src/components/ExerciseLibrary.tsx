...
export function ExerciseLibrary({ selectedExercise, onSelectExercise }: ExerciseLibraryProps) {
  ...
  const filteredExercises = useMemo(() => {
    ...
    return filtered;
  }, [searchTerm, selectedCategory, selectedDifficulty, equipmentFilter, showEquipmentOnly, sortBy]);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Comprehensive database of 100+ exercises with AI-powered form detection
          </p>
        </CardHeader>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredExercises.map((exercise) => (
          <Card key={exercise.id} className={`cursor-pointer transition-all hover:shadow-lg ${
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="text-right">
                  <div className="flex items-center gap-1 text-sm text-muted-foreground">
                    ...
                    {exercise.estimatedCalories} cal
                  </div>
                  <div className="flex items-center gap-1 text-sm text-muted-foreground">
                    <Target className="w-3 h-3" />
                    {exercise.aiAccuracy}% AI
                  </div>
                </div>
              </div>
            </CardHeader>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-primary">
                {Math.round(EXERCISES.reduce((sum, ex) => sum + ex.aiAccuracy, 0) / EXERCISES.length)}%
              </div>
              <div className="text-sm text-muted-foreground">Avg AI Accuracy</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-primary">
                {categories.length - 1}
